...
const ModalEditarCotizacion = ({
  ...
  useEffect(() => {
    if (showModal && cotizacionSeleccionada) {
      setFormData({
        fec_cot: cotizacionSeleccionada.fec_cot,
        est_cot: cotizacionSeleccionada.est_cot,
        validez_dias: cotizacionSeleccionada.validez_dias,
        total_cot: cotizacionSeleccionada.total_cot,
        descuento: cotizacionSeleccionada.descuento,
        notas: cotizacionSeleccionada.notas || "",
        id_cli: cotizacionSeleccionada.id_cli,
        id_emp: cotizacionSeleccionada.id_emp,
      });
      fetchClientes(1, "");
      fetchEmpleados(1, "");
    }
  }, [showModal, cotizacionSeleccionada, fetchClientes, fetchEmpleados]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Error al editar cotización");
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl max-w-5xl w-full max-h-[95vh] overflow-hidden flex flex-col">
        ...
        <div className="bg-gradient-to-r from-blue-500 to-indigo-500 px-6 py-4 flex items-center justify-between">
          <h2 className="text-xl font-bold text-white flex items-center gap-3">
            ...
            Editar Cotización -{" "}
            {cotizacionSeleccionada.cod_cot ||
              `#${cotizacionSeleccionada.id_cot}`}
          </h2>
          <button
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="date"
                    value={formData.fec_cot}
                    onChange={(e) =>
                      setFormData({ ...formData, fec_cot: e.target.value })
                    }
                ...
                <div>
                  ...
                  <select
                    value={formData.est_cot}
                    onChange={(e) =>
                      setFormData({ ...formData, est_cot: e.target.value })
                    }
                    className="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
                  >
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                    <Clock className="w-4 h-4 inline mr-1" /> Validez (días)
                  </label>
                  <input
                    type="number"
                    min="1"
                ...
                <div>
                  ...
                  <input
                    type="number"
                    value={formData.total_cot}
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        total_cot: parseFloat(e.target.value) || 0,
                      })
                    }
                    className="w-full px-4 py-2.5 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white"
                  />
        ...
        <div className="border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 px-6 py-4 flex justify-end gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 px-6 py-2.5 bg-blue-500 hover:bg-blue-600 disabled:bg-gray-400 disabled:cursor-not-allowed text-white rounded-lg font-semibold transition-colors shadow-lg shadow-blue-500/30"
          >
            {isSubmitting ? (
              <>
