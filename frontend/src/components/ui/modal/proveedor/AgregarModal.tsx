...
export default function ModalAgregarProveedor({
  showModal,
  setShowModal,
  setProveedores,
}: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch("http://localhost:8000/api/proveedor", {
        method: "POST",
        headers: {
      ...
      setProveedores((prev) => [...prev, responseData?.data ?? responseData]);
      Swal.fire({
        icon: "success",
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[95vh] overflow-hidden flex flex-col">
        ...
        <div className="border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 px-6 py-4 flex justify-end gap-3">
          <button
            onClick={handleClose}
            className="px-5 py-2.5 bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-800 dark:text-gray-200 rounded-xl font-medium"
          >
            Cancelar
          </button>
          <button
