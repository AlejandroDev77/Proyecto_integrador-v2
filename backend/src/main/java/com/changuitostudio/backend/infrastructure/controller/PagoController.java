...
public class PagoController {
    ...
    public PagoController(ManagePagoUseCase managePagoUseCase) {
        this.managePagoUseCase = managePagoUseCase;
    }

    @GetMapping
    public ResponseEntity<ApiResponse<PageResult<Pago>>> listar(
        ...
        PageResult<Pago> result = managePagoUseCase.listar(page, per_page, filters, sort);
        return ResponseEntity.ok(ApiResponse.success(result));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<Pago>> obtenerPorId(@PathVariable Long id) {
        return managePagoUseCase.obtenerPorId(id)
    ...
    public ResponseEntity<ApiResponse<Pago>> crear(@RequestBody Pago pago) {
        Pago creado = managePagoUseCase.crear(pago);
        return ResponseEntity.status(HttpStatus.CREATED).body(ApiResponse.success(creado));
    }

    @PutMapping("/{id}")
    public ResponseEntity<ApiResponse<Pago>> actualizar(@PathVariable Long id, @RequestBody Pago pago) {
        Pago actualizado = managePagoUseCase.actualizar(id, pago);
        return ResponseEntity.ok(ApiResponse.success(actualizado));
    }

    @DeleteMapping("/{id}")
    public ResponseEntity<ApiResponse<Void>> eliminar(@PathVariable Long id) {
        managePagoUseCase.eliminar(id);
